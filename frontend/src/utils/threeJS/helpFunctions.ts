...
export const getIntersectionWithGrid = (
  gridID: number,
...
export const selectionObject = (
  ...
  if (intersections.length > 0) {
    let filteredIntersections = intersections.filter(
      (item: any) =>
        !item.object.name.includes('layer') &&
        !item.object.name.includes('building') &&
        !item.object.type.includes('Axes') &&
        !item.object.type.includes('Scene')
    )
    if (filteredIntersections.length < 1) return false
    const closestIntersection = filteredIntersections.reduce(
      (min: any, obj: any) => {
...
export const drawLine = (start: IVector3, end: IVector3, scene: THREE.Scene): void => {
  // Erstelle eine Geometrie für die Start- und Endpunkte
  const pointsGeometry = new THREE.BufferGeometry()
  const pointsMaterial = new THREE.PointsMaterial({ color: 0xff0000, size: 0.1 })

  const pointsArray = new Float32Array([start.x, start.y, start.z, end.x, end.y, end.z])

  pointsGeometry.setAttribute('position', new THREE.BufferAttribute(pointsArray, 3))

  // Erstelle die Punkte mit der Geometrie und dem Material
  ...
  const lineArray = new Float32Array([start.x, start.y, start.z, end.x, end.y, end.z])

  lineGeometry.setAttribute('position', new THREE.BufferAttribute(lineArray, 3))

  const line = new THREE.Line(lineGeometry, lineMaterial)
  // Füge Punkte und Linien zur Szene hinzu
  scene.add(points)
  scene.add(line)
}
